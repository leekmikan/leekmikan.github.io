...
function Save(){
	if (window.localStorage) {
		let jprg_easy = JSON.stringify(prg, undefined, 1);
		localStorage.setItem('jprg_easy', jprg_easy);
	}
}
function Load(){
  ...
  document.body.addEventListener('keydown',function(){
	...
	if (window.localStorage) {
		let data = localStorage.getItem('jprg_easy');
		if(data){
			prg = JSON.parse(data);
			Update(0);
		}
		data = localStorage.getItem('jrang_easy');
		if(data){
...
function Reset(){
	if (window.localStorage) {
		...
		if(result){
			localStorage.removeItem('jprg_easy');
			prg = [
			[0,0],
			[0,0],
			[0,0],
...
function mes(event){
		let jrang_easy = JSON.stringify(rang, undefined, 1);
		localStorage.setItem('jrang_easy', jrang_easy);
}
function Get_Star(x){
	if (window.localStorage) {
		let tmp = 0;
