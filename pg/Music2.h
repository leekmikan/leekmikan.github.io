...
Complex* Voicesaver(int len, Complex* cmp, int null, int null2) {
    ...
    for (int i = maxi; i < min(len / 2 - er,2000); i += maxi)
    {
        for (int j = i - er; j < i + er; j++)
        {
            tmp[j] = cmp[j];
    ...
    public:
        ...
        WaveData(int *wd, int msize2,WaveData *source) {
            ...
            for (int i = 0;i < msize2 / 2;++i) {
                this->maxv = max(maxv, abs(wd[i] * 1.0));
            }
            this->ch = source->ch;
        ...
        void ConvolutionReverb(std::string rname, double mix, int disable)
        {
            ...
            for (int i = 0; i < len; ++i)
            {
                max = (int)max(sqrt(cmp2[i].Re * cmp2[i].Re + cmp2[i].Im * cmp2[i].Im), max + 0.0);
            }
        ...
        void ConvolutionReverb2(std::string rname, double mix, int disable)
        {
            ...
            {
                for (int j = 0; j < 2; ++j) {
                    max = (int)max(sqrt(cmp2[j][i].Re * cmp2[j][i].Re + cmp2[j][i].Im * cmp2[j][i].Im), max + 0.0);
                }
            }
        ...
        void Speed(double sp, double pitch)
        {
            ...
            if (abs(sp) >= 0.1)
            {
                ...
                len = (int)TOCH(min(6000 / sp / ovr,6000 / ovr));
                int* rt = new int[ts];
                for (int i = 0;i < ts;i++) {
                    rt[i] = 0;
                }
            ...
            for (int i = 0; i < msize / 2; i += TOCH(block * ovr)) {
                ...
                for (int j = 0; j < tmpd.msize / 2; ++j) {
                    if (sum + j < ws) {
                        ...
                        maxv = max(maxv, abs(rt[sum + j] * 1.0));
                    }
                }
        ...
        void Harmony(double* code,int csize) {
            ...
            for (int i = 0;i < csize;i++) {
                ...
                tmp.Speed(1, code[i]);
                int mj = min(tmp.msize / 2, msize / 2);
        ...
        private:
            ...
            char* ItoB(int* cmp, double mult, int cl)
            {
                ...
                {
                    if (cmp[i] < 0) {
                        rt[i * 2 + 1] = max(cmp[i] * mult, vol_min + 0.0) / 256;
                        rt[i * 2] = cmp[i] * mult - rt[i * 2 + 1] * 256;

                    }
                    else {
                        rt[i * 2 + 1] = min(cmp[i] * mult, vol_max + 0.0) / 256;
                        rt[i * 2] = cmp[i] * mult - rt[i * 2 + 1] * 256;
                    }
                }
                return rt;
